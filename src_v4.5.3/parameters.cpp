...
void parameters::read_param(){
	...
	scale_eph = get(param_map, "scale_eph", 1.);
	...
	if (vpos.size() == 1) error_message("if you want to analyse valley dynamics, at least you need provide positions of two valleys");
	...
	if (material_model == "mos2" || material_model == "gaas"){
		if (!alg.eph_sepr_eh)
			error_message("if material_model is mos2 or gaas, alg_eph_sepr_eh must be true");
		if (pmp.pumpA0 > 0)
			error_message("for model, pump is not allowed");
	}
	...
	if (alg.linearize && alg.ddmdteq)
		error_message("if master equation is linearized, ddmdt_eq is not needed");
	if (alg.linearize && !alg.summode)
		error_message("linearization is only supported when we use the generalized scattering-rate matrices P?");
	if (alg.linearize && (pumpMode == "lindblad" || pumpMode == "coherent"))
		error_message("linearization is not allowed for real-time pump");
	if (alg.linearize && (alg.Pin_is_sparse || alg.sparseP))
		error_message("linearization does not support sparse matrices");
	if (alg.linearize && alg.linearize_dPee)
		error_message("alg_linearize_dPee and alg.linearize cannot be both true");
	if (alg.linearize_dPee && (alg.Pin_is_sparse || alg.sparseP))
		error_message("linearization does not support sparse matrices");
	if (alg.linearize_dPee && !alg.ddmdteq)
		error_message("linearize_dPee must be used ddmdteq");
	if (eep.eeMode == "none" && alg.linearize_dPee)
		error_message("alg_linearize_dPee is only active when e-e scattering is active");
	...
	if (alg.eph_enable && (alg.only_eimp || alg.only_ee))
		error_message("if only e-imp or only e-e, e-ph should not be enabled", "read_param");
