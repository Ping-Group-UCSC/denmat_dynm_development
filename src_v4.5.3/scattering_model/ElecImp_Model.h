...
struct elecimp_model
{
		...
		coul_model(coul_model)
	{
		...
		prefac_A = eip.Z[iD] * sqrt(eip.ni_ionized[iD] * latt->volume);
	...
	void reduce_imsig(mymp *mp){
		...
		if (ionode){
			...
			string fnamesige = "ldbd_imsige_ei_D" + int2str(iD) + eip.impMode[iD] + "_byDMD.out";
			if (exists(fnamesige)) fnamesige = "ldbd_imsige_ei_D" + int2str(iD) + eip.impMode[iD] + "_byDMD_updated.out";
