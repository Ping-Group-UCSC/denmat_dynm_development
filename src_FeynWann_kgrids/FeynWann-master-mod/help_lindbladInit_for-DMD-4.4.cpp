...
double find_mu(double ncarrier, double t, double mu0, std::vector<FeynWann::StateE>& e, int bStart, int bStop){
	...
	while (true){
		ncarrier_new = compute_ncarrier(ncarrier < 0, t, result, Ek, bStop - bStart);
...
double compute_ncarrier(bool isHole, double t, double mu, std::vector<diagMatrix>& Ek, int nb){
	...
	for (int i = 0; i < nb; i++){
		...
		result += isHole ? f - 1 : f; // hole concentration is negative
	}
...
vector3<> compute_bsq(std::vector<FeynWann::StateE>& e, int bStart, int bStop, double degthr, std::vector<diagMatrix> F){
	...
	double sum = 0;
	FILE *fpb;
	if (mpiWorld->isHead()) fpb = fopen("bsq_info.out", "w");
	for (size_t ik = 0; ik < e.size(); ik++){
		...
		if (mpiWorld->isHead()){
			fprintf(fpb, "\nk=(%lg,%lg,%lg):\n", e[ik].k[0], e[ik].k[1], e[ik].k[2]);
			fprintf(fpb, "Ek:");
			for (int i = 0; i < nb; i++) { fprintf(fpb, " %lg", Ek[i]); }
			fprintf(fpb, "\n");
			fprintf(fpb, "dfde:");
			for (int i = 0; i < nb; i++) { fprintf(fpb, " %lg", dfde[i]); }
			fprintf(fpb, "\n");
		}
		for (int id = 0; id < 3; id++){
			...
			if (mpiWorld->isHead()){
				fprintf(fpb, "idir = %d\n", id);
				fprintf_complex_mat(fpb, s.data(), nb, "sfull:");
				fprintf_complex_mat(fpb, sdeg.data(), nb, "sdeg:");
			}
			...
			for (int b = 0; b < nb; b++){
				...
				if (mpiWorld->isHead())
					fprintf(fpb, "b= %d ss= %lg bsq= %lg contrib= %lg\n", b, ss[b], bsq, dfde[b] * bsq);
			}
		}
	}
	if (mpiWorld->isHead()) fclose(fpb);
	return bsq_avg / sum;
}
