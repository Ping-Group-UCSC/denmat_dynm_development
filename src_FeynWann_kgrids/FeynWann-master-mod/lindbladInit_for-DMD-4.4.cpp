...
#include "sparse_matrix_for-DMD-4.4.h"
...
struct LindbladInit
{
		...
		for (size_t ik1 = 0; ik1 < k.size(); ik1++){
			...
			for (const double* E1 = E1begin; E1 < E1end; E1++){ //E1 in active range
				for (const double* E2 = E2begin; E2<E2end; E2++){ //E2 in active range
					double dE = (*E1) - (*E2);
					if (writeU && fabs(dE) < nEphDelta*ePhDelta)
						Econserve = true;
	...
	void fwriteP(FILE *fp1, FILE *fp2, FILE *fp1ns, FILE *fp1s, FILE *fp1i, FILE *fp1j, FILE *fp2ns, FILE *fp2s, FILE *fp2i, FILE *fp2j){
		if (!write_sparseP){
			...
			sP1->write_to_files(fp1ns, fp1s, fp1i, fp1j);
			sparse_mat *sP2 = new sparse_mat(P2, nBandsSel*nBandsSel);
			sP2->write_to_files(fp2ns, fp2s, fp2i, fp2j);
		}
	}
	...
	void compute_eph(){
		...
		if (!write_sparseP){
			fp1 = fopenP("ldbd_P1_lindblad" + shole + ".bin." + convert.str(), "wb"); fp1c = fopenP("ldbd_P1_conventional" + shole + ".bin." + convert.str(), "wb");
			fp2 = fopenP("ldbd_P2_lindblad" + shole + ".bin." + convert.str(), "wb"); fp2c = fopenP("ldbd_P2_conventional" + shole + ".bin." + convert.str(), "wb");
		}
		else{
			...
			for (int b2 = 0; b2 < nBandsSel; b2++){
				...
				if (fabs(dE + wqp) < ethr){
					...
					if (compute_imsig && (ik != jk || b1 != b2)){
						imsig[ik][b1] += prefac_imsig * gp(b1, b2).norm() * (F[jk][b2] + nqp) * deltaplus;
						if (ik < jk) imsig[jk][b2] += prefac_imsig * gp(b1, b2).norm() * (1 - F[ik][b1] + nqp) * deltaplus;
					}
				}
				if (lindblad)
					Gp = G2p;
					...
					Gp = gp(b1, b2);
				App[b1*nBandsSel + b2] = Gp * sqrt(nqp + 1);
				Apm[b1*nBandsSel + b2] = Gp * sqrt(nqp);
				A2pp[b1*nBandsSel + b2] = G2p * sqrt(nqp + 1);
				...
				if (fabs(dE - wqm) < ethr){
					...
					if (compute_imsig && (ik != jk || b1 != b2)){
						imsig[ik][b1] += prefac_imsig * gm(b1, b2).norm() * (1 - F[jk][b2] + nqm) * deltaminus;
						if (ik < jk) imsig[jk][b2] += prefac_imsig * gm(b1, b2).norm() * (F[ik][b1] + nqm) * deltaminus;
					}
				}
				if (lindblad)
					Gm = G2m;
					...
					Gm = gm(b1, b2);
				Amp[b1*nBandsSel + b2] = Gm * sqrt(nqm + 1);
				Amm[b1*nBandsSel + b2] = Gm * sqrt(nqm);
				A2mp[b1*nBandsSel + b2] = G2m * sqrt(nqm + 1);
	...
	void merge_eph_P(){
		...
		if (!write_sparseP){
			...
			merge_files(dir_ldbd + "ldbd_P1_conventional" + shole + ".bin", ctmp, (size_t)std::pow(nBandsSel, 4)); merge_files(dir_ldbd + "ldbd_P2_conventional" + shole + ".bin", ctmp, (size_t)std::pow(nBandsSel, 4));
		}
		else{
	...
	void savekData(){
		...
		if (n_dmu == 0 || fw.isMetal)
			...
			dmu = find_mu(ncarrier, Tmax, n_dmu > 0 ? EcMin : EvMax, state_elec, bStart, bStop);
		}
		F = computeF(Tmax, dmu, state_elec, bStart, bStop);
		double n_dmuMax = compute_ncarrier(dmuMax < Emid, Tmax, dmuMax, state_elec, bStart, bStop) / nkTot / cell_size(fw);
		double n_dmuMin = compute_ncarrier(dmuMin <= Emid, Tmax, dmuMin, state_elec, bStart, bStop) / nkTot / cell_size(fw);
		logPrintf("At dmuMax: "); print_carrier_density(fw, n_dmuMax);
		logPrintf("At dmuMin: "); print_carrier_density(fw, n_dmuMin);
	...
	void relax_rate_usegm(){
		...
		if (writegm){ // storing all gm matrices may need huge memory; we may not want to print it
			...
			if (nEphDelta * ePhDelta / eV / 0.002 > 4) T1_rate_dRho(false, false, true, 0, 0.002);
			if (nEphDelta * ePhDelta / eV / 0.005 > 4) T1_rate_dRho(false, false, true, 0, 0.005);
			if (nEphDelta * ePhDelta / eV / 0.01 > 4) T1_rate_dRho(false, false, true, 0, 0.01);
			...
			if (nEphDelta * ePhDelta / eV / 0.002 > 4) T1_rate_dRho(false, false, true, 1, 0.002);
			if (nEphDelta * ePhDelta / eV / 0.005 > 4) T1_rate_dRho(false, false, true, 1, 0.005);
			if (nEphDelta * ePhDelta / eV / 0.01 > 4) T1_rate_dRho(false, false, true, 1, 0.01);
		}
	}
};
...
int main(int argc, char** argv)
{
	...
	size_t nKeff = k0.size() * fw.eCountPerOffset();
	...
	if (ePhEnabled) lb.kpairSelect(q0);
