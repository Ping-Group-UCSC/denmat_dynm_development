...
struct DebugEph{
	...
	DebugEph(FeynWann& fw, int bandStart, int bandStop, int modeStart, int modeStop)
	: fw(fw), bandStart(bandStart), bandStop(bandStop), modeStart(modeStart), modeStop(modeStop),
	...
	{	const diagMatrix& E1 = mEph.e1->E;
		...
		logPrintf("k^2 & dE: %14.7le %14.7le\n", GGT.metric_length_squared(mEph.e2->k), E2[bandStart] - E1[bandStart]);
		/*
...
{	
	...
	Mtot *= 1./3;
	DebugEph src(fw, bandStart, bandStop, modeStart, modeStop);
	src.Mtot = Mtot;
