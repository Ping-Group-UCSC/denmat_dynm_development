...
void electron::set_H_BS(int ik0_glob, int ik1_glob){
	...
	for (int ik_local = 0; ik_local < nk_proc; ik_local++){
		...
		vec3_dot_vec3array(H_BS[ik_local], B, s[ik_glob], nb_dm*nb_dm); //vec3_dot_vec3array(complex *vm, vector3<complex> v, complex **m, int n);
	}
}
...
void electron::get_kpath(){
	for (int ipath = 0; ipath < nkpath; ipath++){
		...
		for (int ik = 0; ik < nk; ik++){
			vector3<double> krel = kvec[ik] - kpath_start[ipath];
			double length = krel.length();
			if (length > length_kpath) continue;
	...
	if (ionode){
		...
		for (int ik = 0; ik < ik_kpath.size(); ik++)
			printf("%lg %lg %lg\n", kvec[ik_kpath[ik]][0], kvec[ik_kpath[ik]][1], kvec[ik_kpath[ik]][2]);
	}
}
...
double electron::find_mu(double carrier_cell, double t, double mu0){
	if (carrier_cell == 0) error_message("carrier_cell cannot be 0", "electron::find_mu");
	bool isHole = carrier_cell < 0;
	double result = mu0;
	double damp = 0.7, dmu = 5e-6;
	double excess_old_old = 1., excess_old = 1., excess, carrier_cell_new;
	int step = 0;

	while (true){
		carrier_cell_new = compute_nfree(isHole, t, result) + eip.compute_carrier_cell_of_impurity_level(isHole, t, result);
		excess = carrier_cell_new - carrier_cell;
		if (fabs(excess) > 1e-14){
			...
			if (step > 0 && sgn(excess) == sgn(excess_old)){
				double ratio = carrier_cell_new / carrier_cell;
				if (ratio < 1e-9)
