...
void electronphonon::set_eph(){
	...
	set_kpair();
	set_ephmat();
	if (eimp_model != nullptr && !alg.only_ee) add_scatt_contrib("eimp");
	if (ee_model != nullptr && eep.eeMode == "Pee_fixed_at_eq" && !alg.only_eimp) add_scatt_contrib("ee");
	set_sparseP(true);
}
void electronphonon::reset_scatt(bool reset_eimp, bool reset_ee, complex **dm_expand, complex **dm1_expand, double t){
	...
	if ((!reset_eimp && !reset_ee) || (eimp_model == nullptr && ee_model == nullptr)) return;
	...
	set_ephmat();
	if (clp.update) coul_model->init(dm);
	if (eimp_model != nullptr && !alg.only_ee) { if (eimp_model->imsig != nullptr) zeros(eimp_model->imsig, nk_glob, nb); add_scatt_contrib("eimp"); }
	if (ee_model != nullptr && !alg.only_eimp) { if (ee_model->imsig != nullptr) zeros(ee_model->imsig, nk_glob, nb); add_scatt_contrib("ee", dm, dm1, t); }
	set_sparseP(false);
}

void electronphonon::add_scatt_contrib(string what, complex **dm, complex **dm1, double t){
	...
	if (ionode && what == "eimp") printf("\nAdd electron-impurity scattering contribution to P?\n");
	...
	for (int ikpair_local = 0; ikpair_local < nkpair_proc; ikpair_local++){
		...
		if (what == "eimp") eimp_model->calc_P(ik_glob, ikp_glob, P1add, P2add, false);
		if (what == "ee" && DEBUG) ee_model->calc_P_debug(ik_glob, ikp_glob, P1add, P2add, false, dm, dm1, t, ikpair_local == nkpair_proc - 1);
		if (what == "ee" && !DEBUG) nk3_accum += ee_model->calc_P(ik_glob, ikp_glob, P1add, P2add, false, dm, dm1, t, ikpair_local == nkpair_proc - 1);

		complex factor = ((what == "eimp" && alg.only_eimp) || (what == "ee" && alg.only_ee)) ? c0 : c1; // ee must be turned off
	...
	if (what == "ee" && !DEBUG && ionode) printf("nk3_accum: max= %lg min= %lg avg = %lg\n", max, min, avg);

	if (what == "eimp") eimp_model->reduce_imsig(this->mp);
...
void electronphonon::set_sparseP(bool fisrt_call){
	...
	if (alg.sparseP){
		...
		sP2->sparse(P2, true);
		sP1->write_smat("ldbd_data/sP1_" + alg.scatt + "_ns.bin", "ldbd_data/sP1_" + alg.scatt + "_s.bin", "ldbd_data/sP1_" + alg.scatt + "_i.bin", "ldbd_data/sP1_" + alg.scatt + "_j.bin");
		sP2->write_smat("ldbd_data/sP2_" + alg.scatt + "_ns.bin", "ldbd_data/sP2_" + alg.scatt + "_s.bin", "ldbd_data/sP2_" + alg.scatt + "_i.bin", "ldbd_data/sP2_" + alg.scatt + "_j.bin");
	}
}
...
void electronphonon::alloc_ephmat(int ikpair0, int ikpair1){
	...
	if (alg.summode){
		...
		if (!alg.sparseP) P1_next = new complex[(int)std::pow(nb, 4)]{c0};
		if (!alg.sparseP) P2_next = new complex[(int)std::pow(nb, 4)]{c0};
		if (alg.expt) P1t = new complex[(int)std::pow(nb, 4)]{c0};
		if (alg.expt) P2t = new complex[(int)std::pow(nb, 4)]{c0};
	}
	else{
