...
void singdenmat_k::set_dm_eq(bool isHole, double temperature, double mu0, double **e, int bStart, int bEnd){
	...
	if (!isHole) nfree_cell += real(dm[ik][i*nb + i]);
	else nfree_cell += (real(dm[ik][i*nb + i]) - 1.); // hole concentration is negative

	double mu = find_mu(isHole, eip.carrier_cell_ex(isHole, nfree_cell), temperature, mu0, e, bStart, bEnd);
	eip.calc_ni_ionized(isHole, temperature, mu);

	if (!isHole) mue = mu;
	else muh = mu;
}
double singdenmat_k::find_mu(bool isHole, double carrier_cell, double temperature, double mu0, double **e, int bStart, int bEnd){
	double result = mu0;
	...
	while (true){
		carrier_cell_new = compute_nfree_eq(isHole, temperature, result, e, bStart, bEnd) + eip.compute_carrier_cell_of_impurity_level(isHole, temperature, result);
