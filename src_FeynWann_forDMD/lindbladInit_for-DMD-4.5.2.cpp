...
struct LindbladInit
{
			...
			for (int b2 = 0; b2 < nBandsSel; b2++){
				...
				if (fabs(dE + wqp) < ethr && inEwind){
					...
					if (compute_imsig && (ik != jk || b1 != b2)){
						...
						if (ik < jk){
							double dtmp2 = prefac_imsig * gp(b1, b2).norm() * (F[jk][b2] + nqp) * deltaplus;
				...
				if (fabs(dE - wqm) < ethr && inEwind){
					...
					if (compute_imsig && (ik != jk || b1 != b2)){
						const vector3<>& v1 = state_elec[ik].vVec[b1]; const vector3<>& v2 = state_elec[jk].vVec[b2];
						double cosThetaScatter = dot(v1, v2) / sqrt(std::max(1e-16, v1.length_squared() * v2.length_squared()));
						double dtmp1 = prefac_imsig * gm(b1, b2).norm() * (1 - F[jk][b2] + nqm) * deltaminus;
						imsig[ik][b1] += dtmp1; imsigp[ik][b1] += dtmp1 * (1. - cosThetaScatter);
						if (ik < jk){
	...
	void write_imsige(){
		...
		for (size_t ik = 0; ik < k.size(); ik++){
			...
			for (int b = 0; b < nBandsSel; b++){
				...
				const vector3<>& v = state_elec[ik].vVec[b];
	...
	void merge_eph_P_mpi(){
		...
		if (!write_sparseP){
			...
			merge_files_mpi(dir_ldbd + "ldbd_P1_conventional" + shole + ".bin", ctmp, (size_t)std::pow(nBandsSel, 4)); merge_files_mpi(dir_ldbd + "ldbd_P2_conventional" + shole + ".bin", ctmp, (size_t)std::pow(nBandsSel, 4));
		}
		else{
