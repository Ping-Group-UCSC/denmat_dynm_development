...
public:
	...
	bool update_eimp_model_inside(double t){
		return eph->eimp_model != nullptr && pmp.active() && elight->during_pump(t) && param->freq_update_eimp_model < 0;
	}
	bool update_ee_model_inside(){
		return eph->ee_model != nullptr && eep.eeMode == "Pee_update" && param->freq_update_ee_model < 0;
	}
	bool update_eimp_model_outside(double t, int it){
		return eph->eimp_model != nullptr && pmp.active() && elight->during_pump(t) && param->freq_update_eimp_model > 0 && (it-1) % param->freq_update_eimp_model == 0;
	}
	bool update_ee_model_outside(int it){
		return eph->ee_model != nullptr && eep.eeMode == "Pee_update" && param->freq_update_ee_model > 0 && (it-1) % param->freq_update_ee_model == 0;
	}
	...
	void report_tau(int it, string lable = ""){
		...
		if (param->print_along_kpath) ob->ddmk_ana(sdmk->t, sdmk->dm);
	}
};
