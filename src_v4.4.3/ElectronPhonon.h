...
public:
	...
	electronphonon(mymp *mp, lattice *latt, parameters *param, electron *elec, phonon *ph, bool sepr_eh = false, bool isHole = false)
		...
		eimp_model(nullptr), ee_model(nullptr), sP1(nullptr), sP2(nullptr)
	{
		...
		if (eip.ni != 0 && alg.summode)
			eimp_model = new elecimp_model(latt, param, elec, bStart, bEnd, coul_model);
		if (eep.eeMode != "none" && alg.summode)
			ee_model = new elecelec_model(latt, param, elec, bStart, bEnd, coul_model);
	}
	...
	void add_scatt_contrib(string what, complex **dm = nullptr, complex **dm1 = nullptr, double t = 0);
	//void reset_scatt(bool reset_eimp, bool reset_ee, double nfree, complex **dm, complex **dm1, double t);
	void reset_scatt(bool reset_eimp, bool reset_ee, complex **dm, complex **dm1, double t);
