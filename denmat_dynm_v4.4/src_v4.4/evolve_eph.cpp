...
void electronphonon::compute_ddm_eq(double** f0_expand){
	...
	trunc_copy_array(ddm_eq, ddm_expand, nk_glob, nb_expand, bStart, bEnd);
}

void electronphonon::evolve_driver(double t, complex** dm_expand, complex** dm1_expand, complex** ddmdt_eph_expand){
	trunc_copy_array(dm, dm_expand, nk_glob, nb_expand, bStart, bEnd);
	trunc_copy_array(dm1, dm1_expand, nk_glob, nb_expand, bStart, bEnd);
...
void electronphonon::evolve(double t, complex** dm, complex** dm1, complex** ddmdt_eph){
	...
	/*
	string dir = "debug_info/";
	if (ionode) system("mkdir debug_info");
	...
	string fname = dir + "debug.out." + convert.str();
	FILE *fp;
	bool ldebug = fabs(t - 0) < 1e-6;
	...
	*/
	...
	for (int ikpair_local = 0; ikpair_local < nkpair_proc; ikpair_local++){
		...
		if (alg.summode){
			...
			//if (ldebug) fprintf_complex_mat(fp, P1[ikpair_local], nb*nb, "P1[ikpair_local]:");
			//if (ldebug) fprintf_complex_mat(fp, P2[ikpair_local], nb*nb, "P2[ikpair_local]:");
